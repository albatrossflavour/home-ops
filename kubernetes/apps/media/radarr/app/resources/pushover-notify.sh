...
# kubectl port-forward service/radarr -n default 8989:80
...
if [[ "${radarr_eventtype:-}" == "Download" ]]; then
    ...
    printf -v PUSHOVER_MESSAGE "<b>%s (S%02dE%02d)</b><small>\n%s</small><small>\n\n<b>Client:</b> %s</small><small>\n<b>Quality:</b> %s</small>" \
        "${radarr_series_title}" \
        "${radarr_episodefile_seasonnumber}" \
        "${radarr_episodefile_episodenumbers}" \
        "${radarr_episodefile_episodetitles}" \
        "${radarr_download_client}" \
        "${radarr_episodefile_quality}"
    printf -v PUSHOVER_URL "%s/series/%s" "${radarr_applicationurl:-localhost}" "${radarr_series_titleslug}"
    printf -v PUSHOVER_URL_TITLE "View series in %s" "${radarr_instancename:-Radarr}"
fi
